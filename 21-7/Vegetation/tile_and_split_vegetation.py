"""
Tile large raster/vector pairs into smaller chips for vegetation training.
"""
...
OUT_ROOT = Path(r"C:\Users\13519\data\train\vegetation")
...
def get_valid_geometries(gdf):
    """Extract valid Polygon/MultiPolygon geometries."""
    valid_geoms = []
...
def world_to_pixel(geom, left, top, pixel_size_x, pixel_size_y):
    """Translate geometry from world coords to pixel coords."""
...
def tile_raster_vector_pair(raster_path, vector_path, out_root, tile_size, stride):
    ...
    if vector_path.exists():
        try:
            ...
            print(f"  Vector: {len(gdf)} raw -> {len(all_geoms)} valid polygons")
    ...
    for y in range(0, height - tile_size + 1, stride):
        for x in range(0, width - tile_size + 1, stride):
            ...
            # Skip empty tiles (vegetation: no need to keep empty ones)
            if mask.sum() == 0:
                continue
