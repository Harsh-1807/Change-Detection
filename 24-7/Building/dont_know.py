...
if hasattr(os, 'add_dll_directory'):
    # Add conda DLLs first
    conda_dll = r"C:\Users\13519\AppData\Local\anaconda3\envs\lulc\Library\bin"
    if os.path.exists(conda_dll):
        os.add_dll_directory(conda_dll)
...
class DinoV3Segmentor(nn.Module):
    ...
    def load_decoder(self, path):
        """Load only decoder weights from a full-model checkpoint."""
        state = torch.load(path, map_location=DEVICE)

        # Case 1: checkpoint already has a 'decoder' sub-dict
        if 'decoder' in state and isinstance(state['decoder'], dict):
            self.decoder.load_state_dict(state['decoder'])
            return

        # Case 2: checkpoint is wrapped in 'state_dict'
        if 'state_dict' in state:
            state = state['state_dict']

        # Case 3: full model checkpoint with 'decoder.' prefix
        decoder_state = {}
        for k, v in state.items():
            if k.startswith('decoder.'):
                decoder_state[k.replace('decoder.', '', 1)] = v

        if decoder_state:
            self.decoder.load_state_dict(decoder_state)
        else:
            # Fallback: assume flat decoder keys
